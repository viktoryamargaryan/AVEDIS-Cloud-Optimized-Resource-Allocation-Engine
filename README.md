# AVEDIS — Cloud Resource Allocation Engine
### (AVL-Engine-Dijkstra-Scheduler)

> A high-performance resource management system that brings "Good News" (optimal allocation) to complex cloud networks by balancing computational load and minimizing network latency.
...
**AVEDIS** (a play on the Armenian word for "Good News") is an intelligent **resource allocation engine**. It sits inside a data center and makes the most critical decision for any cloud provider: *"Which server should run each incoming job, and what is the most cost-effective way to get it there?"*

The name represents the perfect synergy of its core components: **AV**L Trees for search, a high-speed **E**ngine for dispatch, **DI**jkstra's algorithm for routing, and an automated **S**cheduler.

It answers the allocation problem by combining three data structures into a single, provably optimal pipeline:
...
| Name | GitHub | Role | Primary Contributions |
|------|--------|------|-----------------------|
| **Viktorya Margaryan** | [@viktoryamargaryan](https://github.com/viktoryamargaryan) | Backend & Logic | Designed and implemented the **AVL Tree from scratch** (all 4 rotations, best-fit search, delete). Built the **Allocation Engine** (Hash Map integration), **Flask REST API** (11 endpoints), and the **Web Dashboard**. |
| **Lilit Zalinyan** | [@Lilit862](https://github.com/Lilit862) | Routing & QA | Designed and implemented the **Weighted Graph** and **Dijkstra's algorithm** with full path reconstruction. Built the **Job Simulator**, wrote the **90+ Pytest suite**, and handled project documentation. |
---
